...
В определенный период развития индустрии проектирования и верификации цифровых устройств был предложен подход, добавляющий некоторый уровень абстракции при написании тестового окружения и тестовых сценариев для проверки дизайнов. Такой подход назвали верификацией, основанной на транзакциях или [Transaction-based verification (TBV)](https://github.com/MPSU/SV4DV/tree/master/lab_03%20SystemVerilog%20OOP#transaction-based-verification-tbv).
...
По сути своей UVM стандартизирует архитектуру верификационного ООП-окружения, а также методы конфигурации и взаимодействия его компонентов.
...
Очень важно отметить, что **смена параметра меняет тип**. То есть, `my_param_class #(5)` и `my_param_class #(6)` являются **различными типами**, которые отличаются значением по умолчанию поля `my_data`.

### 2. SystemVerilog параметризация классов типами

|Классы SystemVerilog можно параметризовывать `типами`.|
|:---|
...
В прошлой лабораторной работе вы познакомились с базовыми концепциями ООП, используемыми в UVM, иерархией, созданием UVM-объектов, выводом информации о них. Дело в том, что в верификационном ООП-окружении объекты должны взаимодействовать друг с другом для передачи различной информации.

В `UVM` механизмы взаимодействия реализованы при помощи методологии `Transaction Level Modeling (TLM)`.

|Основной концепцией `TLM` является разделение функциональной составляющей объектов от способов их взаимодействия между собой.|
|:---|

На уровне абстракции `TLM` **взаимодействие между объектами реализовано посредством вызова высокоуровневых методов**, которые содержат в себе информацию более низкого уровня для реализации этого взаимодействия.

Применительно к верификационному окружению, его компоненты обмениваются транзакциями при помощи вызова методов, реализация которых определяет, какие из них связаны между собой, как это влияет на их внутреннее состояние, а также транзакциями какого типа и на каких условиях они обмениваются.

### 4. Основные концепции UVM TLM

**UVM `TLM` определяет взаимодействие** между UVM-объектами.
...
UVM `TLM` предоставляет высокий уровень абстракции и контроля потоков данных между объектами.
...
`imp` реализует отправку и получение транзакции (выполняет запрашивемое от нее действие конкретным способом).

_Пример (`.examples/lab_02/uvm_example_1.sv`)._
...
| `TLM-API` (Application Programming Interface) по своей сути является набором функций (`function`) и задач (`task`), которые реализованы в `UVM` для передачи транзакций между UVM-объектами (в том числе UVM-портами). Подробный разбор методов `TLM-API` представлен [здесь](#72-реализация-tlm-api). |
|:---|
...
_Заметим, что соединение производится в форме `wr2.p_export.connect(wr3.p_export)`, где, очевидно, отдающим является `p_export` класса `Wrapper2`, а берущим на данном уровне иерархии является `p_export` класса `Wrapper3`._
...
Стоит отметить, что использование портов (`port`) при движении через объекты, "обернутые" вокруг инициирующего порта, является неким негласным соглашением. **При таком движении также можно использовать экспорты (`export`), кроме инициирующего порта**.
...
Между UVM-объектами должна передаваться транзакция:
